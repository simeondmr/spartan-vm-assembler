...
impl GrammarProductionParsing<(), VariableInfo> for SingleVarDecl {
    fn parse(&self, _param: Option<()>) -> Result<VariableInfo, AssemblerErrors> {
        ...
        let mut lexer = <SingleVarDecl as GrammarProductionParsing<_, _>>::lexer_lock();
        let current_token = lexer.current_token();
        <SingleVarDecl as GrammarProductionParsing<_, _>>::match_token(&Token::NumberU32(0, 0), &mut lexer)?;
        ...
        let variable_info =  semantic_analyzer::semantic_analyzer::check_var_declaration(current_token.line(), <SingleVarDecl as GrammarProductionParsing<_, _>>::symbol_table().lock().unwrap().insert_variable(variable_name, type_size, array_size, array_size * type_size))?;
        <SingleVarDecl as GrammarProductionParsing<_, _>>::codegen().lock().unwrap().alloc_space(variable_info.total_size() as usize);
